...
from raphiia_openai.operational import accounting_store, inventory_store, pcdoctor_store, party_store, procurement_store, personal_ledger
...
def gcp_budget_create(
    ...
    return ag44.gcp_budget_create(billing_account_id, display_name, amount, threshold_percents=threshold_percents, credit_types_treatment=credit_types_treatment, dry_run=dry_run, approval_id=approval_id)
...
def gcp_project_iam_add_binding(project_id: str, member: str, role: str, dry_run: bool = True, approval_id: str = "") -> dict[str, Any]:
    """AG-44: agrega IAM binding minimo con allowlist y approval gate."""
    from raphiia_openai.agents import ag44_cloud_deployer as ag44
...
def cloudflare_status() -> dict[str, Any]:
    """Estado credenciales Cloudflare (DNS/WAF), túnel y capacidades de aprovisionamiento."""
    from raphiia_openai import cloudflare_ops

    return cloudflare_ops.cloudflare_status()


@mcp.tool
def cloudflare_provision_subdomain(
    subdomain: str,
    service: str,
    waf_skip: bool = True,
    proxied: bool = True,
    ...
    """Crea subdominio en pcdoctor.ai: CNAME proxied + ingress túnel + WAF skip + verify HTTPS."""
    from raphiia_openai import cloudflare_ops

    return cloudflare_ops.provision_subdomain(
        subdomain,
        service,
        waf_skip=waf_skip,
        proxied=proxied,
...
@mcp.tool
def record_ledger_transaction(payload: dict[str, Any]) -> dict[str, Any]:
    """Registra un movimiento en el ledger financiero personal y operativo."""
    return personal_ledger.record_transaction(payload)


@mcp.tool
def query_ledger(filters: dict[str, Any], limit: int = 100) -> dict[str, Any]:
    """Consulta agregable del ledger financiero personal."""
    return personal_ledger.query_ledger(filters, limit)


@mcp.tool
def resolve_account_alias(alias: str) -> dict[str, Any] | None:
    """Resuelve un alias natural de cuenta al catálogo canónico."""
    return personal_ledger.resolve_account_alias(alias)
...
def generate_photo(
    ...
    """Genera foto fotorrealista local (RealVisXL ultra, ~42 pasos).

    aspect: 16:9 | 9:16 | 1:1
    quality: ultra (default, lento, máxima calidad) | standard
    prompt: descripción adicional; si vacío usa brief/title
    """
    from raphiia_openai.video_pipeline.photo import generate_photo as _gen
