...
def _gcp_billing_allowed(billing_account_id: str) -> bool:
    ...
    return bool(allowed and billing_account_id in allowed)
...
def _gcp_candidate(action: str, validation: dict[str, Any], command: list[str], *, mutates: bool) -> dict[str, Any]:
    ...
    if validation.get("ok") and not validation.get("dry_run"):
        result["ok"] = False
        result["error"] = "gcp_apply_executor_pending_racb_lock"
        result["hint"] = "The public apply surface is gated; command execution remains closed until RACB/idempotency executor is wired."
    return result
...
def _cloud_owner_action(provider: str, project_id: str, scopes: list[str], risk_level: str) -> dict[str, Any]:
    ...
    if provider == "gcp":
        return {
            "summary": "Authorize Google Cloud on the server, not in chat.",
            "human_steps": [
                "Open an owner-controlled terminal/session on the target server or provider console.",
                "Authenticate gcloud or install a service account using the secure owner vault path.",
                "Set project/region env server-side and allowlist the exact project/billing account.",
                "Return only the request_id plus confirmation; do not paste keys or tokens.",
            ],
            "server_env_expected": [
