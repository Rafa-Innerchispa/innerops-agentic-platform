...
function stopResponse(){
  ...
  hideTyping();busy=false;btnSend.disabled=false;inputEl.disabled=false;
  updateStopBtn();
...
async function loadTtsVoices(){
  try{
    ...
    const def=d.default||'';
    settingVoice.value=settings.voice||def||(ttsVoices[0]&&ttsVoices[0].id)||'';
  }catch(e){}
}
...
def voice_tts_voices():
    ...
    default = str(tts.PIPER_MODEL.resolve()) if tts.PIPER_MODEL.is_file() else (voices[0]["id"] if voices else "")
...
def voice_tts_samples_status(user: dict[str, Any] = Depends(voice_auth.require_user)):
    ...
    uname = str(user.get("username") or "user").lower()
    folder = VOICE_SAMPLES_ROOT / uname
