...
def run_ag14(message: str = "", **_: Any) -> dict[str, Any]:
    from raphiia_openai.operational import pcdoctor_store
    name = message.strip() or "Nuevo cliente"
    return _ok("AG-14", "client_draft", **pcdoctor_store.create_client_draft(name=name))
...
def _import_dedicated() -> dict[str, Runner]:
    from raphiia_openai.agents import (
        ...
        ag56_sandbox_fleet_agent as ag56,
    )
    ...
    return {
        ...
        "AG-56": lambda message="", **kw: _merge("AG-56", "sandbox", ag56.agent_sandbox_dispatch(message or "estado", node=kw.get("node", "amd"))),
    }
