...
from raphiia_openai import coordination_docs, editorial_media_upload, editorial_publish, editorial_store, funding_registry as funding_registry_module, image_gen, linkedin_client, local_execution_plane, local_model_router, mcp_diagnostics, mongo_store
...
def cloud_deploy_status() -> dict[str, Any]:
    """AG-44: estado providers cloud (GCP/Alibaba/Cloudflare) — stub v1."""
...
def route_agent_request(
    ...
    """Resuelve agente y opcionalmente ejecuta. auto_execute=true registra salud, brief, etc."""
...
def dispatch_local_agent(
    ...
    """AG-49: entrada única — guardian|quote|report|invoice|daily|health|iskcon|hackathon|funding|vero."""
    from raphiia_openai.agents import ag49_local_dispatcher as ag49
...
def save_owner_credential(
    ...
    """Guarda credencial cifrada solo para Rafael (owner vault)."""
    from raphiia_openai import owner_vault

    return owner_vault.save_owner_credential(
        key=key, secret=secret, category=category, label=label, metadata=metadata
    )


@mcp.tool
def get_owner_credential(key: str, category: str = "", reveal: bool = True) -> dict[str, Any]:
    """Recupera credencial del owner vault (solo Rafael)."""
    from raphiia_openai import owner_vault

    return owner_vault.get_owner_credential(key, category=category, reveal=reveal)


@mcp.tool
def list_owner_credentials(category: str = "", reveal: bool = False) -> dict[str, Any]:
    """Lista claves del owner vault; reveal=true muestra secretos."""
    from raphiia_openai import owner_vault

    return owner_vault.list_owner_credentials(category=category, reveal=reveal)


@mcp.tool
def get_owner_vault_summary() -> dict[str, Any]:
    """Resumen de bóveda de credenciales de Rafael."""
    from raphiia_openai import owner_vault

    return owner_vault.get_owner_vault_summary()


@mcp.tool
def import_owner_credentials_from_email() -> dict[str, Any]:
    """Importa email_accounts + email_secrets.json a bóveda cifrada."""
    from raphiia_openai import owner_vault

    return owner_vault.import_email_accounts_to_vault()


@mcp.tool
def microsoft_mail_oauth_status() -> dict[str, Any]:
    """Estado OAuth Microsoft para Hotmail/ESPOL."""
    from raphiia_openai.notifications import microsoft_mail_oauth as ms

    return ms.oauth_config_status()


@mcp.tool
def start_microsoft_mail_oauth(email: str, tenant: str = "common") -> dict[str, Any]:
    """Inicia login OAuth (device code). Rafael abre URL en Windows."""
    from raphiia_openai.notifications import microsoft_mail_oauth as ms

    return ms.start_device_login(email=email, tenant=tenant)


@mcp.tool
def complete_microsoft_mail_oauth(session_id: str, timeout_sec: int = 300) -> dict[str, Any]:
    """Completa login OAuth tras autenticación en navegador."""
    from raphiia_openai.notifications import microsoft_mail_oauth as ms

    return ms.complete_device_login(session_id, timeout_sec=timeout_sec)
...
def backfill_inbox_reviews(
    ...
    """Re-clasifica todos los correos monitorizados (v3). Alertas WhatsApp solo si send_alerts=true."""
    from raphiia_openai.notifications import email_review

    return email_review.backfill_inbox_reviews(
        limit=limit,
...
@mcp.tool
def get_email_security_status() -> dict[str, Any]:
    """Estado política seguridad correo (adjuntos bloqueados, ClamAV, cuarentena)."""
    from raphiia_openai.notifications import email_security

    return email_security.get_security_status()


@mcp.tool
def list_email_actions(status: str | None = None, limit: int = 20) -> dict[str, Any]:
    """Cola de acciones de correo enrutadas a agentes locales."""
    from raphiia_openai.notifications import email_router

    return email_router.list_email_actions(status=status, limit=limit)


@mcp.tool
def list_captured_email_documents(doc_type: str | None = None, limit: int = 20) -> dict[str, Any]:
    """Facturas/cotizaciones capturadas desde correo (claves SRI, metadatos)."""
    from raphiia_openai.notifications import email_document_capture

    return email_document_capture.list_captured_documents(doc_type=doc_type, limit=limit)


@mcp.tool
def get_email_capture_summary() -> dict[str, Any]:
    """Resumen documentos capturados del correo."""
    from raphiia_openai.notifications import email_document_capture

    return email_document_capture.get_capture_summary()


@mcp.tool
def confirm_email_routing(
    mail_id: str,
    agent_id: str,
    module: str = "",
    next_step: str = "",
) -> dict[str, Any]:
    """Confirma enrutamiento de un correo — el sistema aprende para remitentes similares."""
    from raphiia_openai.notifications import email_router

    return email_router.confirm_routing(mail_id, agent_id, module=module, next_step=next_step)


@mcp.tool
def get_accounting_ledger_status() -> dict[str, Any]:
    """Estado libro contable unificado (Contifico + email + RalfIA)."""
    from raphiia_openai.operational import accounting_ledger

    return accounting_ledger.get_ledger_status()


@mcp.tool
def sync_accounting_ledger_from_contifico(limit: int = 5000) -> dict[str, Any]:
    """Sincroniza contifico_documents → ralfia_ledger_documents."""
    from raphiia_openai.operational import accounting_ledger

    return accounting_ledger.sync_contifico_to_ledger(limit=limit)


@mcp.tool
def search_accounting_ledger(
    query: str | None = None,
    year: int | None = None,
    month: int | None = None,
    ...
    """Busca documentos contables tabulados (facturas, retenciones, cotizaciones)."""
    from raphiia_openai.operational import accounting_ledger

    return accounting_ledger.search_ledger_documents(
        query=query,
        year=year,
        month=month,
...
@mcp.tool
def get_accounting_ledger_monthly(year: int, month: int | None = None) -> dict[str, Any]:
    """Resumen mensual: facturas recibidas, emitidas, retenciones, totales."""
    from raphiia_openai.operational import accounting_ledger

    return accounting_ledger.monthly_ledger_summary(year=year, month=month)


@mcp.tool
def sync_full_accounting_pipeline(contifico_limit: int = 500) -> dict[str, Any]:
    """Import Contifico + normalizar + ledger + capturas email."""
    from raphiia_openai.operational import accounting_ledger

    return accounting_ledger.daily_contifico_pipeline(incremental_pages=max(10, contifico_limit // 50))


@mcp.tool
def run_daily_contifico_pipeline(incremental_pages: int = 10) -> dict[str, Any]:
    """Pipeline diario: Contifico incremental, ledger, FAC↔COT, inventario."""
    from raphiia_openai.operational import accounting_ledger

    return accounting_ledger.daily_contifico_pipeline(incremental_pages=incremental_pages)


@mcp.tool
def import_contifico_incremental(pages: int = 10, size: int = 50, dry_run: bool = False) -> dict[str, Any]:
    """Sync incremental documentos recientes desde Contifico API."""
    from raphiia_openai import contifico_bridge

    return contifico_bridge.import_contifico_incremental(pages=pages, size=size, dry_run=dry_run)


@mcp.tool
def link_fac_to_cot_quotes(limit: int = 500) -> dict[str, Any]:
    """Relaciona facturas emitidas con cotizaciones Contifico del mismo cliente."""
    from raphiia_openai.operational import accounting_ledger

    return accounting_ledger.link_fac_to_cot_quotes(limit=limit)


@mcp.tool
def sync_inventory_from_contifico(hydrate_max: int = 80) -> dict[str, Any]:
    """Rellena inventario local desde catálogo Contifico."""
    from raphiia_openai.operational import accounting_ledger

    return accounting_ledger.sync_inventory_from_contifico(hydrate_max=hydrate_max)


@mcp.tool
def peek_next_contifico_invoice_number(tipo_documento: str = "FAC") -> dict[str, Any]:
    """Próximo número factura compatible Contifico (continuidad al migrar)."""
    from raphiia_openai.operational import accounting_ledger

    return accounting_ledger.peek_next_contifico_number(tipo_documento=tipo_documento)
...
@mcp.tool
def get_sandbox_steward_status(node: str = "amd") -> dict[str, Any]:
    """Estado sandbox uncensored: modelos Ollama :11436 y propuestas pendientes de borrado."""
    from raphiia_openai import sandbox_steward

    return sandbox_steward.build_status(node=node)


@mcp.tool
def propose_sandbox_model_delete(model_name: str, reason: str = "", node: str = "amd") -> dict[str, Any]:
    """Propone borrar un modelo del sandbox — NO ejecuta hasta que Rafael confirme por WhatsApp."""
    from raphiia_openai import sandbox_steward

    return sandbox_steward.propose_delete_model(model_name, node=node, reason=reason or None, requested_by="mcp")


@mcp.tool
def notify_sandbox_event(message: str, severity: str = "info") -> dict[str, Any]:
    """Notifica a Rafael por WhatsApp un evento del sandbox (despliegue, error, cambio)."""
    from raphiia_openai import sandbox_steward

    return sandbox_steward.notify_owner(message, severity=severity)


@mcp.tool
def agent_sandbox_status(node: str = "amd") -> dict[str, Any]:
    """AG-56: estado sandbox uncensored (Ollama + WebUI + modelos). Cero créditos cloud."""
    from raphiia_openai.agents import ag56_sandbox_fleet_agent as ag56

    return ag56.agent_sandbox_status(node=node)


@mcp.tool
def agent_sandbox_search_models(query: str, limit: int = 10) -> dict[str, Any]:
    """AG-56: busca modelos en Ollama.com + locales sandbox."""
    from raphiia_openai.agents import ag56_sandbox_fleet_agent as ag56

    return ag56.agent_sandbox_search_models(query, limit=limit)


@mcp.tool
def agent_sandbox_install_model(pull_name: str, alias: str = "", node: str = "amd") -> dict[str, Any]:
    """AG-56: pull + alias uncensored en sandbox local. Notifica WhatsApp."""
    from raphiia_openai.agents import ag56_sandbox_fleet_agent as ag56

    return ag56.agent_sandbox_install_model(pull_name, alias or None, node=node)


@mcp.tool
def agent_sandbox_fix_webui(node: str = "amd") -> dict[str, Any]:
    """AG-56: repara WebUI sandbox (think=false, historial, modelos pinned)."""
    from raphiia_openai.agents import ag56_sandbox_fleet_agent as ag56

    return ag56.agent_sandbox_fix_webui(node=node)


@mcp.tool
def agent_sandbox_dispatch(message: str = "", node: str = "amd") -> dict[str, Any]:
    """AG-56: router NL — estado, buscar, instalar, fix webui, proponer borrado."""
    from raphiia_openai.agents import ag56_sandbox_fleet_agent as ag56

    return ag56.agent_sandbox_dispatch(message, node=node)
...
def video_pipeline_health() -> dict[str, Any]:
    """Estado del pipeline de vídeo (TTS, ComfyUI, ffmpeg)."""
...
def generate_video_content(
    ...
    """Genera vídeo narrado (TTS + escenas + ffmpeg). aspect: 9:16|16:9|1:1."""
