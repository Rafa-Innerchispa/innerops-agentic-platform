...
READER_ERROR_ALERT_AFTER = int(os.getenv("SOLAR_READER_ERROR_ALERT_AFTER", "3"))
...
def reader_error_line(data: dict[str, Any], streak: int) -> str:
    parts = [
    ...
    if data.get("ssh_target"):
        parts.append(f"target={data.get('ssh_target')}")
    if data.get("remote_reader"):
        parts.append(f"reader={data.get('remote_reader')}")
    last_good = data.get("last_good") or {}
    if last_good:
        parts.append(f"last_good_age={last_good.get('cache_age_seconds')}s")
    stderr = " ".join(str(data.get("stderr") or "").split())
    if stderr:
...
def build_events(data: dict[str, Any], state: dict[str, Any], now: datetime) -> list[dict[str, Any]]:
    if not data.get("ok"):
        streak = int(state.get("reader_error_streak") or 0) + 1
        if streak < READER_ERROR_ALERT_AFTER:
            return []
        return [{
            "event": "reader_error",
            "severity": "warning",
            "message": f"InnerOS Solar: no pude leer el inversor Xmart Pi01. {reader_error_line(data, streak)}",
...
def update_conditions(data: dict[str, Any], state: dict[str, Any], now: datetime) -> None:
    ...
    if data.get("ok"):
        state["reader_error_streak"] = 0
        state["last_reader_ok_at"] = now.isoformat()
    else:
        state["reader_error_streak"] = int(state.get("reader_error_streak") or 0) + 1
        state["last_reader_error"] = {
            "at": now.isoformat(),
            "error": data.get("error") or "unknown",
            "returncode": data.get("returncode"),
            "ssh_target": data.get("ssh_target"),
            "remote_reader": data.get("remote_reader"),
            "stderr": str(data.get("stderr") or "")[-500:],
        }
