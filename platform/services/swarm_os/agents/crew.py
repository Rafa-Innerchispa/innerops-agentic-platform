...
try:
    ...
    build_agents = get_llm = None
try:
    from tools.crew_tools import get_inspection_context
except ModuleNotFoundError:
    get_inspection_context = None
...
def run_inspection_flow(raw_input: str, inspection_id: str | None = None) -> dict[str, Any]:
    ...
    """
    if Crew is None:
        result = run_inspection_demo(raw_input, inspection_id=inspection_id)
        result["fallback_reason"] = "crewai_not_available"
        return result
