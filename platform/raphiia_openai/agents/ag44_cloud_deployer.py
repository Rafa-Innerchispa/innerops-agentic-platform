...
PROVIDERS = {
    ...
    "cloudflare": {
        "label": "Cloudflare",
        "status": "partial",
        "targets": ["tunnel_systemd", "dns_api", "workers"],
        "cli": "cloudflared / API",
        "note": "Túnel systemd operativo; API DNS pendiente",
    },
}
...
def cloud_deploy_status() -> dict[str, Any]:
    ...
    return {
        ...
        "next": [
            "Terraform módulo mínimo Cloud Run en ralphiia-ecosystem-core/infra/",
            "Tool cloud_deploy_apply con gate humano + manage_coordination_lock",
            "Alibaba adapter reutilizando misma interfaz",
        ],
    }
