...
def detect_tool_calls(user: dict[str, Any], text: str) -> list[tuple[str, dict[str, Any]]]:
    ...
    if "raul_dispatch" in perms and re.search(
        ...
        calls.insert(
            ...
            (
                "raul_dispatch",
                {"message": text[:2000], "channel": "voice"},
            ),
        )

    if "vero_dispatch" in perms and re.search(
        r"\b(vero|avero|facturador|dile\s+a\s+vero|dile\s+a\s+avero)\b", t, re.I
    ):
        calls.insert(
            0,
            (
