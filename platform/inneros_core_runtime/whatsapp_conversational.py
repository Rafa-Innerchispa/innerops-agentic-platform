...
_INTENT_TOOLS: list[tuple[re.Pattern[str], str]] = [
    (re.compile(r"\b(pendientes?|proyectos?|backlog|ideas?\s+pendientes?|olvidados?)\b", re.I), "backlog"),
    ...
    if "pcdoctor_store.list_clients" in sources or (
        ...
        if _CLIENT_LIST_RE.search(message) or re.search(r"\b(mis\s+clientes?|lista\s+de\s+clientes?)\b", message, re.I):
            ...
            if block:
                ...
                shown_m = re.search(r"(\d+)\s+mostrados", block)
                legacy_m = re.search(r"PC Doctor legacy=(\d+)", block)
                cont_m = re.search(r"Contífico=(\d+)", block)
                shown = shown_m.group(1) if shown_m else "varios"
                legacy_n = legacy_m.group(1) if legacy_m else "?"
                cont_n = cont_m.group(1) if cont_m else "?"
                examples = names_m.group(1).strip() if names_m else ""
                if "CLIENTES Contífico" in block:
                    body = (
                        f"En Contífico hay {cont_n} contactos marcados como cliente. "
                        f"Te muestro {shown}."
                    )
                else:
                    body = (
                        f"Tienes {legacy_n} clientes operativos PC Doctor (edificios, urbanizaciones, servicios). "
                        f"Te muestro {shown}."
                    )
                    if cont_n and cont_n != "?":
                        body += f" Además hay {cont_n} en Contífico (contabilidad; incluye facturaciones puntuales)."
                if examples:
                    body += f" Ejemplos: {examples[:280]}."
                body += " Pregúntame por uno concreto (facturas, saldo banco, cotizaciones)."
                return body
        ...
        if identity and identity.get("authenticated") and "owner" in set(identity.get("roles") or []):
            ...
            if client_list_intent:
                try:
                    ...
                    contifico_scope = bool(
                        re.search(r"\b(cont[ií]fico|contabilidad|facturaci[oó]n)\b", message, re.I)
                    )
                    listing = pcdoctor_store.list_clients(
                        limit=40,
                        scope="contifico" if contifico_scope else "pcdoctor",
                    )
                    names = [
                        m.get("display_name") or m.get("name") or m.get("client_name") or m.get("client_id", "?")
                        for m in (listing.get("matches") or [])[:15]
                    ]
                    label = "Contífico" if listing.get("scope") == "contifico" else "PC Doctor"
                    blocks.append(
                        f"CLIENTES {label}: {listing.get('count', 0)} mostrados de "
                        f"{listing.get('total_legacy') if listing.get('scope') == 'pcdoctor' else listing.get('total_contifico', 0)} "
                        f"(PC Doctor legacy={listing.get('total_legacy')}, Contífico={listing.get('total_contifico', 0)}). "
...
def _format_vero_reply(result: dict[str, Any]) -> str:
    if result.get("reply_text"):
        return _trim(str(result["reply_text"]))
    if result.get("action") == "proactive_briefing" and result.get("proactive"):
        from raphiia_openai.commercial import vero_proactive_advisor
        return _trim(vero_proactive_advisor.format_proactive_reply(result["proactive"]))
    if not result.get("ok"):
        detail = result.get("detail") or result.get("error") or "No pude completar la solicitud."
        examples = result.get("examples") or []
        body = f"*Vero · {detail}*"
        if examples:
            body += "\n\nEjemplos:\n" + "\n".join(f"• {ex}" for ex in examples[:3])
        return _trim(body)
    parts = [f"*Vero · {result.get('action', result.get('intent', 'listo'))}*"]
    if result.get("display_number"):
        parts.append(f"Cotización: `{result['display_number']}`")
    if result.get("quote_id"):
        parts.append(f"ID: `{result['quote_id']}`")
    if result.get("total"):
        parts.append(f"Total: ${result['total']:,.2f}")
    if result.get("report_id"):
        parts.append(f"Informe: `{result.get('display_number') or result['report_id']}`")
    if result.get("receivable_id"):
        parts.append(f"Cobro AR: `{result['receivable_id']}`")
    if result.get("approval_required"):
        parts.append("⏳ Pendiente tu confirmación para emitir factura fiscal.")
    fiscal = result.get("fiscal") or {}
    if fiscal.get("message"):
        parts.append(str(fiscal["message"]))
    if result.get("mission_id"):
        parts.append(f"Misión: `{result['mission_id']}`")
    return _trim("\n".join(parts))
...
def _fallback(message: str, intents: list[str]) -> str:
    ...
    if "backlog" in intents:
        return "Escribe *pendientes* para ver proyectos/ideas o *desarrolla 1* para asignar el primero a los agentes."
    return (
...
def conversational_reply(
    ...
    try:
        ...
        if raul_orchestrator.mentions_raul(text_in):
            result = raul_orchestrator.raul_dispatch(text_in, channel="whatsapp", phone=sender)
            body = raul_orchestrator.format_raul_reply(result)
            if sender:
                _save_turn(sender, "user", text_in, conversation_id=conversation_id, is_group=is_group, entity_id=entity_id)
                _save_turn(
                    sender,
                    ...
                    is_group=is_group,
                    entity_id=entity_id,
                    metadata={"source": "raul_dispatch", "intent": result.get("intent")},
                )
            return {"ok": result.get("ok", False), "text": body, "source": "raul", "result": result}
    except Exception:
        pass
    ...
    try:
        ...
        if vero_orchestrator.mentions_vero(text_in):
            result = vero_orchestrator.vero_dispatch(text_in, channel="whatsapp", phone=sender)
            body = _format_vero_reply(result)
            if sender:
                _save_turn(sender, "user", text_in, conversation_id=conversation_id, is_group=is_group, entity_id=entity_id)
                _save_turn(
                    sender,
                    ...
                    is_group=is_group,
                    entity_id=entity_id,
                    metadata={"source": "vero_dispatch", "mission_id": result.get("mission_id")},
                )
            return {"ok": result.get("ok", False), "text": body, "source": "vero", "result": result}
    except Exception:
        pass
    ...
    # RalfIA dispatch local — agentes AG antes de Ollama genérico
    try:
        from raphiia_openai.whatsapp_ralfia_bridge import try_ralfia_dispatch_wa

        rd = try_ralfia_dispatch_wa(text_in, identity=identity, dry_run=False)
        if rd and rd.get("text"):
            ...
            if sender:
                ...
                _save_turn(
                    ...
                    is_group=is_group,
                    entity_id=entity_id,
                    metadata={"source": "ralfia_dispatch", "routed": rd.get("routed", {}).get("agent_id")},
                )
            return {**rd, "text": body, "intents": intents, "had_context": bool(context)}
    except Exception:
        pass
