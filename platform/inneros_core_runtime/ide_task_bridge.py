...
# ACP-compatible projection API. These helpers intentionally sit beside the
# durable dispatch_task lifecycle above so callers can inspect delivery vs
# execution without creating a Mongo-backed ops task.
...
def normalize_target(target: str) -> str:
    raw = str(target or "").strip().lower().replace(" ", "-")
    aliases = {
        "chatgpt": "gemini",
        "chatgpt-a": "gemini",
        "chatgpt_a": "gemini",
    ...
    return normalize_ide(aliases.get(raw, raw))


def _acp_idempotency_key(*, target: str, correlation_id: str, title: str) -> str:
...
def project_execution_state(
    *,
    ...
    """Map inbox/A2A/RACB state into IDE contract (delivery is not execution)."""
...
def dispatch_ide_task(
    *,
    ...
    """Create an ACP-style dry-run dispatch record with tracking envelope."""
    target_id = normalize_target(target)
    if target_id not in SUPPORTED_IDES:
        return {"ok": False, "error": "unsupported_ide", "target": target, "supported": sorted(SUPPORTED_IDES)}
    ...
    idem = _acp_idempotency_key(target=target_id, correlation_id=corr, title=title)
    ...
    message = {
        ...
        "metadata": {"repo": repo, "branch": branch, "worktree": worktree, "traceparent": envelope["traceparent"]},
    }
    ...
    projection = project_execution_state(a2a_status=a2a_stub, ops_status="proposed", target=target_id)
    record = {
...
def mark_ops_progress(
    *,
    ...
    projection = project_execution_state(a2a_status=a2a_status, ops_status=ops_status, target=record["target"])
