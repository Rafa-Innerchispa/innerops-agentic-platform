...
def _import_dedicated() -> dict[str, Runner]:
    ...
    return {
        ...
        "AG-44": lambda message="", **kw: _merge("AG-44", "cloud_deploy", ag44.cloud_deploy_plan("cloudflare", message) if "cloudflare" in (message or "").lower() or "pcdoctor.ai" in (message or "").lower() else ag44.cloud_deploy_status()),
...
# --- AG-45 structured local-dev fanout v2 (2026-08-23) ---
# Overrides the legacy message-as-repo runner above. Supports JSON commands while
# preserving plain repo inspection for backward compatibility.
def run_ag45(message: str = "", **kw: Any) -> dict[str, Any]:
    ...
    if not raw:
        return _ok("AG-45", "local_exec_ready", ready=True, contract={
            "inspect": {"repo": "owner/name"},
            "fanout": {"action": "fanout", "repo": "owner/name", "task_ids": ["ops_x"], "concurrency": 6},
        })
    ...
    if raw.startswith("{"):
        try:
            ...
            return _ok("AG-45", "local_exec_invalid_json", ok=False, error=f"invalid_json:{exc}")

    # Backward-compatible plain repo inspection.
    if payload is None:
        repo = raw
        return _ok("AG-45", "local_exec_inspect", **local_execution_plane.local_exec_inspect_repo(repo))
    ...
    if not repo or "/" not in repo:
        return _ok("AG-45", "local_exec_contract", ok=False, error="repo_must_be_owner_name", payload=payload)

    if action == "inspect":
        return _ok("AG-45", "local_exec_inspect", **local_execution_plane.local_exec_inspect_repo(repo))

    if action != "fanout":
        return _ok("AG-45", "local_exec_contract", ok=False, error=f"unsupported_action:{action}")
    ...
    if not task_ids:
        return _ok("AG-45", "local_dev_fanout", ok=False, error="task_ids_required")

    policy = local_execution_plane.repo_policy_status(repo)
    if not policy.get("ok") or policy.get("write_scope") in {"none", "read_only"}:
        return _ok("AG-45", "local_dev_fanout", ok=False, error="repo_policy_denied", policy=policy)

    fanout = dev_swarm_scheduler.fanout_execute(repo=repo, task_ids=task_ids, concurrency=concurrency, dry_run=False)
    return _ok(
        "AG-45",
        "local_dev_fanout",
        ...
        canonical_entrypoint="dev_swarm_scheduler.fanout_execute",
        fanout=fanout,
    )


# Replace registry binding created earlier in this module.
POOL_RUNNERS["AG-45"] = run_ag45
