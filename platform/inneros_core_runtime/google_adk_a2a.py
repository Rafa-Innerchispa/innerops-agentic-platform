...
def _card_to_remote_spec(agent_id: str, card: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "protocol_version": card.get("protocolVersion") or a2a_bridge.PROTOCOL_VERSION,
...
def google_gemini_remote_card() -> dict[str, Any]:
    ...
    return {
        ...
        "protocolVersion": a2a_bridge.PROTOCOL_VERSION,
...
def remote_a2a_agents() -> dict[str, Any]:
    ...
    cards = dict(a2a_bridge.agent_cards().get("cards") or {})
...
def dispatch_remote_sub_agent(
    ...
    *,
    ...
    if agent_id not in (a2a_bridge.agent_cards().get("cards") or {}):
    ...
    dispatch_fn = getattr(bridge, "dispatch", None) if bridge is not None else None
    if dispatch_fn is None:
        dispatch_fn = a2a_bridge.dispatch
    result = dispatch_fn(
        agent_id=agent_id,
        title=title,
        body=body,
    ...
    return {
        **result,
        ...
        "live_mode": LIVE_MODE,
        "a2a_task_id": (result.get("task") or {}).get("a2a_task_id") or result.get("a2a_task_id"),
        "ops_task_id": (result.get("task") or {}).get("ops_task_id") or result.get("ops_task_id"),
    }
