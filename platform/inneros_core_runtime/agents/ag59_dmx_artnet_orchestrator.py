...
_RAW_ADDRESS_TOKENS = {"channel", "channels", "canal", "canales", "universe", "universo"}
...
def _scene_identifier(scene: str) -> str | None:
    """Normalize a high-level scene name while rejecting raw DMX addressing."""
    raw = _normalize(scene).replace("_", " ")
    normalized = "_".join(raw.split())
    if not normalized or len(normalized) > 80:
        return None
    if any(ch not in "abcdefghijklmnopqrstuvwxyz0123456789_" for ch in normalized):
        return None
    if set(normalized.split("_")).intersection(_RAW_ADDRESS_TOKENS):
        return None
    return normalized
...
def _sanitize_backend_scenes(raw: dict[str, Any]) -> list[str]:
    scenes: list[str] = []
    seen: set[str] = set()
    for value in list(raw.get("supported_scenes") or []) + list(SAFE_SCENES):
        safe = _scene_identifier(str(value))
...
def _backend_scene_catalog() -> tuple[set[str], dict[str, Any]]:
    """Fetch the backend-advertised high-level scene catalog, fail-closed."""
    raw = _request_json("GET", "/api/status")
    if not raw.get("ok"):
        return set(), raw
    return set(_sanitize_backend_scenes(raw)), raw
...
def dmx_status() -> dict[str, Any]:
    ...
    if not raw.get("ok"):
        return {
            ...
            "agent_id": AGENT_ID,
            "status": "unavailable",
            "error": raw.get("error", "dmx_backend_unavailable"),
        }
    return {
        "ok": True,
        "agent_id": AGENT_ID,
        ...
        "safe_scenes": _sanitize_backend_scenes(raw),
...
def dmx_set_scene(
    ...
    *,
    ...
    """Apply a backend-advertised high-level scene or safe color.

    Dynamic scene names are executable only when the local bounded DMX backend
    advertises the exact normalized identifier in ``supported_scenes``. Raw
    channels, universes and arbitrary effects remain unavailable.
    """
    ...
    advertised_scenes: list[str] = list(SAFE_SCENES)

    if canonical is None and safe_color is None:
        candidate = _scene_identifier(scene)
        if candidate is not None:
            advertised, catalog_raw = _backend_scene_catalog()
            if not catalog_raw.get("ok"):
                return {
                    "ok": False,
                    "agent_id": AGENT_ID,
                    "error": catalog_raw.get("error", "dmx_backend_unavailable"),
                }
            advertised_scenes = sorted(advertised)
    ...
    if canonical is None and safe_color is None:
        return {
            ...
            "allowed_scenes": advertised_scenes,
    ...
    if canonical not in {None, "morado_uv", "rojo_sangre"}:
