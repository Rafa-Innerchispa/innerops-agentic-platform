...
def htr_record_from_productivity(payload: dict[str, Any] | None) -> dict[str, Any]:
    ...
    mode = "MEASURED" if measurement_class == "measured" and verified else "ESTIMATED"
    ...
    if mode == "MEASURED" and not source:
...
def routing_evidence_from_resource_decision(
    ...
    *,
    ...
    provider_id = _clean(model.get("provider_id") or provider.get("provider_id"))
    selected_model = _clean(model.get("model_provider") or model.get("model_ref") or provider.get("label") or provider_id)
    cost_policy = _clean(model.get("cost_policy") or provider.get("cost_policy"))
    local_cloud = "cloud" if cost_policy == "explicit_burst_only" or provider.get("kind") in {"cloud", "cloud_provider"} else "local"
    return {
        "schema_version": "inneros.routing_evidence.v1",
        "provider_id": provider_id,
        "provider_kind": _clean(provider.get("kind")),
        "selected_model": selected_model,
        "selected_agent": _clean(provider.get("label") or provider_id),
        "local_cloud": local_cloud,
        "route_reason": "resource_fabric_selected" if selected else "resource_fabric_no_candidate",
