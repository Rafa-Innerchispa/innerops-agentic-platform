...
class AuditFabricTests(unittest.TestCase):
    ...
    def test_measured_htr_requires_verified_source(self):
        htr = audit_fabric.htr_record_from_productivity(
            {
                "task_key": "measured-source",
                "human_baseline_minutes": 45,
                "assisted_minutes": 10,
                "measurement_class": "measured",
                "verified": True,
                "measurement_source": "timer:v1",
            }
        )

        self.assertEqual(htr["measurement_mode"], "MEASURED")
        self.assertEqual(htr["measurement_source"], "timer:v1")
        self.assertIsNone(htr["estimate_reason"])

    def test_measured_htr_without_source_is_estimated(self):
        htr = audit_fabric.htr_record_from_productivity(
            {
                "task_key": "measured-no-source",
                "human_baseline_minutes": 45,
                "assisted_minutes": 10,
                "measurement_class": "measured",
                "verified": True,
        ...
        self.assertIsNone(htr["measurement_source"])
        self.assertEqual(htr["estimate_reason"], "measured productivity claim missing measurement_source")
    ...
    def test_estimated_htr_is_explicit_when_not_verified(self):
        ...
        self.assertEqual(htr["measurement_mode"], "ESTIMATED")
        self.assertEqual(htr["estimate_reason"], "measured productivity claim not verified")
        self.assertEqual(htr["quality_gate"]["gate"], "estimated")
    ...
    def test_resource_route_emits_routing_evidence_without_blocking_selection(self):
        ...
        self.assertEqual(routing["reason_codes"], ["local_first", "capacity_available"])
        self.assertEqual(routing["fallback"], False)
        self.assertEqual(routing["policy"], "resource_fabric_v1")
        self.assertEqual(routing["policy_version"], "2026-09-09")

    def test_routing_evidence_preserves_live_resource_fabric_fields(self):
        routing = audit_fabric.routing_evidence_from_resource_decision(
            {
                ...
                "selected": {
                    ...
                    "provider": {"provider_id": "local-amd-5", "kind": "local_node", "label": "Local AMD .5"},
                    "fallback": False,
                },
                "candidates": [1, 2],
            },
        ...
        self.assertEqual(routing["selected_model"], "QuantTrio/Qwen3-Coder-30B-A3B-Instruct-AWQ")
        self.assertEqual(routing["provider_kind"], "local_node")
        self.assertEqual(routing["reason_codes"], ["local_first", "capacity_available"])
        self.assertEqual(routing["fallback"], False)
        self.assertEqual(routing["latency_ms"], 12.5)
        self.assertEqual(routing["candidate_count"], 2)
