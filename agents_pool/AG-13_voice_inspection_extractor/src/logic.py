# Lógica CrewAI para voice_inspection_extractor (AG-13)
...
def run(context_variables):
    ...
    with open(os.path.join(base_dir, "config", "agent.yaml"), "r") as f:
        agent_config = yaml.safe_load(f)["voice_inspection_extractor"]
        
    with open(os.path.join(base_dir, "config", "tasks.yaml"), "r") as f:
        task_config = yaml.safe_load(f)["extract_voice_findings"]
