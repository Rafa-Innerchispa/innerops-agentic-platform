# Lógica CrewAI para hub_dashboard_generator (AG-20)
...
def run(context_variables):
    ...
    with open(os.path.join(base_dir, "config", "agent.yaml"), "r") as f:
        agent_config = yaml.safe_load(f)["hub_dashboard_generator"]
        
    with open(os.path.join(base_dir, "config", "tasks.yaml"), "r") as f:
        task_config = yaml.safe_load(f)["compile_dashboard_metrics"]
