# Lógica CrewAI para notion_cosmos_orchestrator (AG-07)
...
def run(context_variables):
    ...
    with open(os.path.join(base_dir, "config", "agent.yaml"), "r") as f:
        agent_config = yaml.safe_load(f)["notion_cosmos_orchestrator"]
        
    with open(os.path.join(base_dir, "config", "tasks.yaml"), "r") as f:
        task_config = yaml.safe_load(f)["sync_databases"]
